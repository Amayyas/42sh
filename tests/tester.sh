...
{
  id=$1
  $CAT test_function | $GREP -A1000 "\[$id\]" | $GREP -B1000 "^\[$id-END\]" | $GREP -v "^\[.*\]"
}
...
load_test()
{
  ...
  then
    if [ $debug -ge 1 ]
    then
      echo -e "\e[33mTest $id\e[0m \e[34m($NAME)\e[0m : \e[32mOK\e[0m"
      if [ $debug -eq 2 ]
      then
        echo "Output $MYSHELL :"
        $CAT -e /tmp/.shell.$$
    ...
    if [ $debug -ge 1 ]
    then
      echo -e "\e[33mTest $id\e[0m \e[34m($NAME)\e[0m : \e[31mKO\e[0m - Check output in /tmp/test.$$/$id/"
...
if [ $# -eq 0 ]
then
  for lst in `cat test_function | grep "^\[.*\]$" | grep -vi end | sed s/'\['// | sed s/'\]'//`
  do
    path_backup=$PATH
    load_test $lst 1
